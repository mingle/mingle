...
class AddCardNumberToDependencyResolvingCards < ActiveRecord::Migration[5.0]
  def self.up
  ...
  def self.update_dependency_resolving_cards_table_for(dependency_type)
    dep_resolving_cards = exec_query sanitize_sql("SELECT * FROM #{safe_table_name('dependency_resolving_cards')} WHERE #{quote_column_name('dependency_type')}='#{dependency_type.name}'")
    dep_resolving_cards.each do |entry|
      result = exec_query(sanitize_sql(%{
