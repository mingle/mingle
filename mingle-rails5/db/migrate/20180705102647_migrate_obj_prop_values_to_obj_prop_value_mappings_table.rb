...
class MigrateObjPropValuesToObjPropValueMappingsTable < ActiveRecord::Migration[5.0]
  class << self
    alias_method :c, :quote_column_name
    alias_method :t, :safe_table_name

    def up
      objectives = select_all sanitize_sql("SELECT #{c('id')}, #{c('program_id')}, #{c('value')}, #{c('size')} FROM #{t('objectives')}")
