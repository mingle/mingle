...
class AddDefaultObjectivePropertyDefinitions < ActiveRecord::Migration[5.0]
  class << self
    ...
    def up
      ...
      program_ids.each do |program_id_hash|
        ...
        size_id = select_one(sanitize_sql("SELECT id FROM #{t('objective_property_definitions')} WHERE name = 'size' AND #{c('program_id')} = ?", program_id))['id']
        value_id = select_one(sanitize_sql("SELECT id FROM #{t('objective_property_definitions')} WHERE name = 'value' AND #{c('program_id')} = ?", program_id))['id']
