...
class AddRaisingCardNumberToDependency < ActiveRecord::Migration[5.0]
  class << self
    ...
    def iterate_over(table, &block)
      (exec_query sanitize_sql("SELECT #{c("id")}, #{c("raising_project_id")}, #{c("raising_card_id")} FROM #{t(table)}")).each do |entry|
