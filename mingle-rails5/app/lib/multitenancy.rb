  ...
  class << self
    ...
    def find_tenant(name)
      if config = tenant_config(name)
        Rails.logger.info "Tenant config for #{name}  : #{config.symbolize_keys.except(:database_password)}"
    ...
    def remove_deprecated_configs(config)
      accepted_keys = ENV_CONFIG + FEATURE_TOGGLES
    ...
    def missing_schemata_tenants
      @missing ||= KeyValueStore.create MingleConfiguration.missing_schemata_dynamodb_table, :tenant, :configs
    end
