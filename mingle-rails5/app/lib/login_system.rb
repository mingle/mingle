...
module LoginSystem
  ...
  def sign_in
    ...
    if (request.post?) || (session[:redirected_to_external_authentication])
      begin
        if (user = self.authenticate)
          flash[:notice]  = 'Sign in successful'
          redirect_back_or_default(user)
        else
          ApplicationController::TransactionFilter.set_rollback_only
  ...
  def sign_out(logout_redirect_to=nil)
    ...
    MingleSession.find_by_session_id(request.session_options[:id]).try(:destroy)
  ...
  def request_api_uri?
    request.path =~ /\A(#{CONTEXT_PATH})?\/api\//
  end

  def load_user_from_session_or_cookie
    session[:init] = true
    (session[:login] && User.find_by_login(session[:login], :user_display_preference)) || load_user_from_cookie
  end
  ...
  def protect_brute_force_attack(user, &block)
    ...
    try_login_count = Cache.get(counter_key, 0, true).to_i
    if try_login_count >= BRUTE_FORCE_ATTEMPT_LIMIT
      ...
      yield.tap do |user|
        if user.is_a?(User) || session[:login]
          ...
          Cache.incr counter_key, 1, BRUTE_FORCE_TIMEOUT
        end
      end
    end
  end
  ...
  def access_denied
    unless performed?
      ...
      respond_to do |format|
        format.html { redirect_to login_profile_path }
  ...
  # TODO: Add when oauth support is added, based on plugin. See card [mingle/#1694] Move oauth2 plugin? for more details
  ...
  def basic_authorization_header
    %w(REDIRECT_REDIRECT_X_HTTP_AUTHORIZATION REDIRECT_X_HTTP_AUTHORIZATION X-HTTP_AUTHORIZATION HTTP_AUTHORIZATION Authorization).each do |key|
