      ...
      def need_install?
        ...
        begin
          if Database.need_config?
            data_base_configured = DatabaseHelper.new.setup
            logger.info('Need install: database configuration') unless data_base_configured
            return !data_base_configured
          end
          if Database.need_migration?
            logger.info('Need install: db migration')
            return true
          end
          unless SmtpConfiguration.load
