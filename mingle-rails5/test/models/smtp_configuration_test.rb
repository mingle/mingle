...
class SmtpConfigurationTest < ActiveSupport::TestCase
  ...
  def test_should_test_false_if_minimal_smtp_settings_not_present
    assert_nil SmtpConfiguration.test(valid_settings_params)

    assert_equal ['SMTP server address'], SmtpConfiguration.test(valid_settings_params.merge('smtp_settings' => nil)).blank_fields

    assert_equal ['SMTP server address'], SmtpConfiguration.test(valid_settings_params.merge('smtp_settings' => {'domain' => 'booya.com', 'port' => '25'})).blank_fields

    assert_equal ['SMTP server port'],
                 SmtpConfiguration.test(valid_settings_params.merge('smtp_settings' => {'address' => '192.168.1.2', 'domain' => 'booya.com'})).blank_fields
  end
  ...
  def test_should_give_proper_error_message_when_logged_in_user_does_not_have_email_address
    ...
    assert_equal 'Unable to test email settings. Please go to your profile page to specify an email address and try again.', SmtpConfiguration.test(valid_settings_params).error
  end
