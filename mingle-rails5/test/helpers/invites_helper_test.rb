...
class InvitesHelperTest < ActionView::TestCase
  ...
  def setup
    ...
    @member = create(:user, email: 'member@email.com')
    @project_admin =  create(:user, login: :project_admin)
  end
  ...
  def test_invite_link_disabled_for_admins_on_other_projects_and_smtp_configured
    other_project_admin= create(:user)
    create(:project) do |other_project|
