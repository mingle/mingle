...
class ThreadLocalCacheTest < ActiveSupport::TestCase
  ...
  def test_single_cache_entry_with_key
    ...
    assert_nil ThreadLocalCache.get("ddd")
  end

  def test_get_assn
    user = create(:user)
    u = ThreadLocalCache.get_assn(user.login_access, :user)
    assert_equal user, u

    assert_equal u.object_id, ThreadLocalCache.get_assn(user.login_access, :user).object_id
    assert_equal u, ThreadLocalCache.get_assn(user, :login_access, :user)
  end
