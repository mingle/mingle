...
class Group < ActiveRecord::Base
  ...
  validates_format_of :name, :message => "cannot contain comma.", :with => /^[^,]+$/, :if => Proc.new { |group| group.name.length > 0 }

  named_scope :team do
    {:conditions => {:internal => true}}
  end

  named_scope :user_defined do
    {:conditions => {:internal => false}}
  end

  named_scope :named_like, lambda { |term|
