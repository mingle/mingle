...
class UserDisplayPreference < ActiveRecord::Base
  ...
  def enqueue_recent_users(user)
    return if user.blank?
    recent_users = Array(read_preference(:recent_users))
    recent_users.delete(user.id)
    recent_users.unshift(user.id)
    update_preference(:recent_users,
                      recent_users.first(MAX_RECENT_USER_COUNT * user.projects.count))
  end
