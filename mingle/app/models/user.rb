...
class User < ActiveRecord::Base
  default_scope :conditions => {:system => false}

  include Messaging::MessageProvider
  include HistorySubscriber
  include Deletion
  include UserSearch
  ...
  PERMISSION_FIELDS = [:admin, :light, :activated]
  SENSITIVE_FIELDS = PERMISSION_FIELDS + [:password, :password_confirmation, :lost_password_key, :last_login_at]

  has_many :history_subscriptions,  :dependent => :destroy
  has_many :asynch_requests,        :dependent => :destroy
  has_many :murmurs_read_counts,    :dependent => :destroy
  ...
  has_many :user_filter_usages
  has_one :user_engagement, :dependent => :destroy
  has_many :todos, :dependent => :destroy
  has_many :dependency_views, :dependent => :destroy
  has_many :murmurs, foreign_key: 'author_id'
  has_many :program_dependency_views, :dependent => :destroy
  ...
  validates_format_of     :login, :with => /^[.+@_\w-]+$/
  validates_uniqueness_of :login
  validates_length_of     :login, :within => 1..255
  validates_presence_of   :login
  before_validation       :downcase_login
  validates_presence_of  :name
  validates_exclusion_of :name, :in => [PropertyType::UserType::CURRENT_USER], :message => "cannot be set to #{PropertyType::UserType:: CURRENT_USER.bold}"
  ...
  validates_format_of :password, :with => /[\W_]/, :message => 'needs at least one non-character symbol (e.g. ".", "," or "-")',
    :if => Proc.new {|user| user.password_changed? && Authenticator.strict_password_format?}
  validates_format_of :password, :with => /\d/, :message => 'needs at least one digit',
    :if => Proc.new {|user| user.password_changed? && Authenticator.strict_password_format?}
  ...
  named_scope :admins, :conditions => { :admin => true }, :order => 'lower(name)'
  named_scope :all
  named_scope :order_by_name, :order => 'lower(name)'
  named_scope :all_with_icon, :conditions => ['icon is not ?', nil]
  ...
  class << self
    ...
    def find_by_email(email, *args, &block)
      find(:first, :conditions => ["lower(email) = ?", email.downcase]) unless email.blank?
    end

    # with_exclusive_scope to include system users
    def find_from_ids(*args)
      with_exclusive_scope { super }
    end

    # with_exclusive_scope to include system users
    def find_by_login(login, options={})
      with_exclusive_scope { find(:first, options.merge(:conditions => {:login => login})) }
    end

    # with_exclusive_scope to include system users
    def find_by_id(id, options={})
      with_exclusive_scope { find(:first, options.merge(:conditions => {:id => id})) }
    end
    ...
    def activated_users
      count :conditions => ['activated = ?', true]
    end

    def activated_full_users
      count :conditions => activated_full_users_conditions
    end
    ...
    def activated_light_users
      count :conditions  => ['activated = ? and light = ?', true, true]
    end
    ...
    def first_admin
      admins.find(:first, :order => 'id asc') or raise("Unable to find admin user.")
    end
    ...
    def authenticate(login, password)
      ...
      if user = User.find_by_login(login.downcase)
        ...
        with_exclusive_scope do
          first(:conditions => { :login => login.downcase, :password => sha_password(user.salt, password) })
        end
      end
    end
  ...
  def role
    return 'Mingle Admin' if admin?
    light? ? 'Light User': 'Full User'
  end
  ...
  def admin_in_any_project?
    !User.connection.select_values(self.member_roles.project_admin_ids_sql).empty?
  end
  ...
  def project_ids
    projects.find(:all, :select => :id).collect(&:id)
  end
  ...
  def deliverables(model)
    ...
    model.scoped({:conditions => [%{
      #{model.quoted_table_name}.id IN (
        SELECT team.deliverable_id
        FROM #{UserMembership.quoted_table_name} user_memberships
        JOIN #{Group.quoted_table_name} team ON user_memberships.group_id = team.id AND team.internal = ?
  ...
  def admin_update_profile(values)
    if User.current.id == self.id && has_permission_attributes?(values)
      errors.add_to_base("You cannot update your own user permission attributes.")
      return false
    end
  ...
  def password_changed?
   self.new_record? || self.password != User.find(self.id).password
  end
  ...
  def member_of?(project)
    project.auto_enroll_enabled? || projects.include?(project)
  end

  def membership_role_for(project)
    return nil unless member_of?(project)
    if projects.include?(project)
      member_roles.detect{|mr| mr.deliverable_id == project.id}.role.name
    else
      project.autoenroll_role_for(self).name
    end
  end
  ...
  def icon_url(options)
    if view_helper = options[:view_helper]
      ...
      view_helper.image_url(user_icons.url_for(self))
    end
  end
  ...
  def to_xml_with_other_attributes(options = {})
    write_boolean_attributes
    if options[:with_last_login_time]
      self.last_login_at = self.login_access.last_login_at
    end

    to_xml_without_other_attributes(options)
  end
  alias_method_chain :to_xml, :other_attributes
  ...
  def projects_visible_to(user)
    return self.accessible_projects if self == user
    self.accessible_projects.select{ |project| project.admin?(user) }
  end
  ...
  def validate
    if sole_admin?
      errors.add_to_base("Administrator #{name} cannot be removed as they are the last admin") if !admin?
      errors.add_to_base("Administrator #{name} cannot be deactivated as they are the last admin") if !activated?
    end
    validate_license
  end
  ...
  def crypt_password
    if password_changed?
      write_attribute :salt, SecureRandomHelper.random_32_char_hex
  ...
  def validate_license
    ...
    if self.changed.include?('light') && !light? && !formula2
      errors.add_to_base(license_warning_message(@registration))
      return
    end

    if new_record? || self.changed.include?('activated')
      activated_user_size = self.class.count(:conditions  => ['activated = ?', true]) + 1 # we need count self as activated
      ...
      if light?
        ...
        if not (formula1 && formula3)
          errors.add_to_base(license_warning_message(@registration))
        end
      else
        # if it's full, validate formula (2) and (3)
        if not (formula2 && formula3)
          errors.add_to_base(license_warning_message(@registration))
        end
      end
    end
