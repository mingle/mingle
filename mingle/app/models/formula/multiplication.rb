...
class Formula
  class Multiplication < Formula::Operator
    ...
    def reason_operation_is_invalid
      "a #{@operand1.describe_self_with_type} cannot be multiplied by a #{@operand2.describe_self_with_type}"
    end

    def to_sql(table_name = Card.table_name, cast_to_integer = false, property_overrides = {})
      output_type.to_multiplication_sql(operands, table_name, property_overrides)
    end
    ...
    def accept(operation)
      operation.visit_multiplication_operator(self, *operands)
