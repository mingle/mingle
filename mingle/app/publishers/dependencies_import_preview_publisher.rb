...
class DependenciesImportPreviewPublisher
  include Messaging::Base
  attr_reader :asynch_request

  def initialize(user, zip_file)
    @asynch_request = user.asynch_requests.create_dependencies_import_preview_asynch_request(Time.now.strftime("%Y-%m-%d"), zip_file)
    @message = message(user)
  end

  def publish_message
    send_message(DependenciesImportPreviewProcessor::QUEUE, [@message])
    @message.dup
  end
