...
require 'profile_server/user_observer'
...
module ProfileServer
  class << self
    def configure(options, http=nil)
      ...
      @observer = UserObserver.new(self)
      User.add_observer(@observer)
    end

    def configured?
      @observer != nil && @client != nil
    end
    ...
    def update_last_data_exported_on(timestamp)
      @client.update_last_data_exported_on(timestamp)
    end
    ...
    # test only
    def reset
      User.delete_observer(@observer) if @observer
      @client = nil
      @observer = nil
    end
  end
