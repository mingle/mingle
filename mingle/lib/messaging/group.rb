  ...
  module Group
    module MailboxExt
      def self.included(base)
        base.class_eval do
          alias_method_chain :send_message, :message_group
        end
      end
      def send_message_with_message_group(queue_name, messages)
        ...
        send_message_without_message_group(queue_name, messages)
      end
    end
    ...
    module GatewayExt
      def self.included(base)
        base.class_eval do
          alias_method_chain :receive_message, :message_group
        end
      end

      def receive_message_with_message_group(*args, &block)
        receive_message_without_message_group(*args) do |message|
          MessageGroup.processing(message, &block)
        end
      end
    end
  end
