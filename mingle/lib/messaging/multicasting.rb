...
module Messaging
  module Multicasting
    def self.included(base)
      base.class_eval do
        alias_method_chain :send_message, :multicasting
      end
    end

    def send_message_with_multicasting(queue_name, messages, &block)
      ...
      if route = Messaging::Redirects.detect { |from, targets| from == queue_name }
        ...
        send_message_without_multicasting(queue_name, messages, &block)
      end
    end
