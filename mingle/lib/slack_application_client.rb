...
class SlackApplicationClient
  ...
  def list_all_channels(tenant_name, mingle_team_id)
    params = {tenantName: tenant_name, mingleTeamId: mingle_team_id}
    request = signed_get_request('listAllChannels', params)
    execute(request, params).body
  end
  ...
  def signed_delete_request(path, body)
    ...
    request = Net::HTTP::Delete.new(request_url.to_s)
  ...
  def signed_get_request(path, query_params)
    ...
    request = Net::HTTP::Get.new(request_url.to_s)
    @signer.sign(request)
  end

  def signed_post_request(path, body)
    request_url = URI.parse("#{base_url}/#{path}")
    request = Net::HTTP::Post.new(request_url.to_s)
    request.content_type = 'application/json'
