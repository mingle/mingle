...
class M20100304032206Formula
  class M20100304032206FormulaParser < Racc::Parser
    def unquote(value)
      case value
      when /^'(.*)'$/ then $1
      when /^"(.*)"$/ then $1
  ...
  class Formula::CardPropertyValue < Formula
    ...
    def property_definition
      M20100304032206Project.current.property_definitions.find_by_name(@property_name)
    end
  end
...
class M20100304032206FormulaPropertyDefinition < ActiveRecord::Base
  set_table_name "#{ActiveRecord::Base.table_name_prefix}property_definitions"
  self.inheritance_column = 'M20100304032206_type' #disable single table inheretance
  belongs_to :project, :class_name => 'M20100304032206Project', :foreign_key => 'project_id'
...
class M20100304032206Project < ActiveRecord::Base
  set_table_name "#{ActiveRecord::Base.table_name_prefix}projects"
  has_many :property_definitions, :class_name => 'M20100304032206FormulaPropertyDefinition', :foreign_key => 'project_id'
  
  cattr_accessor :current
end

class SaveIsNumericFlagForFormulaPropertyDefinitions < ActiveRecord::Migration
  def self.up
    parser = M20100304032206Formula::M20100304032206FormulaParser.new
    
    M20100304032206Project.all.each do |project|
      M20100304032206Project.current = project
