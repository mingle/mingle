#  Copyright 2020 ThoughtWorks, Inc.
#  
#  This program is free software: you can redistribute it and/or modify
#  it under the terms of the GNU Affero General Public License as
#  published by the Free Software Foundation, either version 3 of the
#  License, or (at your option) any later version.
#  
#  This program is distributed in the hope that it will be useful,
#  but WITHOUT ANY WARRANTY; without even the implied warranty of
#  MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
#  GNU Affero General Public License for more details.
#  
#  You should have received a copy of the GNU Affero General Public License
#  along with this program.  If not, see <https://www.gnu.org/licenses/agpl-3.0.txt>.

class AddConstraintToTaggingsTable < ActiveRecord::Migration
  def self.up
    change_column :taggings, :tag_id, :integer, :null => false
    change_column :taggings, :taggable_id, :integer, :null => false
    change_column :taggings, :taggable_type, :string, :null => false
  end
  
  def self.down
    change_column :taggings, :tag_id, :integer, :null => true
    change_column :taggings, :taggable_id, :integer, :null => true
    change_column :taggings, :taggable_type, :string, :null => true    
  end
end
