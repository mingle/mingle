...
class M2009091506314Project < ActiveRecord::Base
  set_table_name "#{ActiveRecord::Base.table_name_prefix}projects"
  has_many :property_definitions, :class_name => 'M2009091506314PropertyDefinition', :foreign_key => 'project_id'
  has_many :history_subscriptions, :class_name => 'M2009091506314HistorySubscription', :foreign_key => 'project_id'
  has_many :card_list_views, :class_name => 'M2009091506314CardListView', :foreign_key => 'project_id'
end

class M2009091506314PropertyDefinition < ActiveRecord::Base
  set_table_name "#{ActiveRecord::Base.table_name_prefix}property_definitions"
  self.inheritance_column = 'M2009091506314_type' #disable single table inheretance
  belongs_to :project, :class_name => 'M2009091506314Project', :foreign_key => 'project_id'
  def quoted_column_name
    "#{Project.connection.quote_column_name(column_name)}"
  end
end

class M2009091506314HistorySubscription < ActiveRecord::Base
  
  set_table_name "#{ActiveRecord::Base.table_name_prefix}history_subscriptions"
  belongs_to :project, :class_name => 'M2009091506314Project', :foreign_key => 'project_id'
end

class M2009091506314CardListView < ActiveRecord::Base
...
class AllowProjectToBeUsedAsColumnInMql < ActiveRecord::Migration
  ...
  class << self
    ...
    def fix_formula_usage(project, to_name)
      formula_paser = M2009091506314Formula::FormulaParser.new
      project.property_definitions.find_all_by_type('FormulaPropertyDefinition').each do |formula_prop_def|
        ...
        formula.rename_property('project', to_name)
        formula_prop_def.formula = formula.to_s[1..-2]
        formula_prop_def.save!
      end
    end
    
    def fix_subscription_usage(project, to_name)
      project.history_subscriptions.each do |history_subscription|
        if history_subscription.filter_params =~ /\[project\]=/i
          history_subscription.filter_params.gsub!(/\[project\]=/i, "[#{to_name}]=")
          filter_params = parse_str_params(YAML.load(history_subscription.filter_params).to_s)
          history_subscription.hashed_filter_params = serialize(filter_params).to_yaml.md5
          history_subscription.save!
        end
      end
    end
    
    def fix_changes_usage(to_name)
      ActiveRecord::Base.connection.execute <<-SQL
        UPDATE #{safe_table_name('changes')} 
        SET field='#{to_name}' 
        WHERE type='PropertyChange' AND LOWER(field)='project'
      SQL
    end
    
    def fix_filter_usage(project, to_name)
      project.card_list_views.each do |view|
        filter_string = view.params
        if filter_string =~ /\bproject\b/im
          filter_string.gsub! /\[project\]\[([^\]]*)\]\[([^\]]*)\]/im do |match|
             "[#{to_name}][#{$1}][#{$2}]"
          end
          filter_string.gsub!(/:color_by: project/i, ":color_by: #{to_name}")
          filter_string.gsub!(/:group_by: project/i, ":group_by: #{to_name}")
          filter_string.gsub!(/:columns: (.*)$/) do |match|
            columns_match = $1
            columns = columns_match.split(',')
            columns = columns.collect {|column| column.trim.downcase == 'project' ? to_name : column }
            ":columns: #{columns.join(',')}"
          end
            
          view.params = filter_string
          
          view.canonical_string.gsub! /\[project\]\[([^\]]*)\]\[([^\]]*)\]/im do |match|
             "[#{to_name}][#{$1}][#{$2}]"
          end
          view.canonical_string.gsub!(/color_by=project/im, "color_by=#{to_name}")
          view.canonical_string.gsub!(/group_by=project/im, "group_by=#{to_name}")
          view.canonical_string = view.canonical_string.split(',').collect do |part|
            if part.downcase == 'project'
              to_name
            elsif part.downcase == 'columns=project'
              "columns=#{to_name}"
            else
              part
            end
    ...
    def serialize(filter_params)
      if str = ActionController::Routing::Route.new.build_query_string(filter_params)[1..-1]
        URI.unescape(str)
      end
    end
  ...
  def self.up
    M2009091506314Project.find(:all).each do |project|
      conflict_property_definition = project.property_definitions.detect do |prop_def|
        prop_def.name.downcase == 'project'
      end
      ...
      if conflict_property_definition
        ...
        fix_formula_usage(project, to_name)
        fix_changes_usage(to_name)
        fix_subscription_usage(project, to_name)
        fix_filter_usage(project, to_name)
      end
    end
  end
