...
class AddNumberSequenceToBacklogObjectives < ActiveRecord::Migration
  class << self
    ...
    def up
      ...
      programs = execute sanitize_sql("SELECT #{c('id')} FROM #{t('deliverables')} WHERE #{c('type')} = '#{Program.name}'")
    ...
    def create_number_sequence(program_id, last_value)
      table_sequence = execute sanitize_sql("SELECT id FROM #{t('table_sequences')} WHERE #{c('name')} = '#{sequence_name(program_id)}'")
