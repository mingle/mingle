  ...
  module ConnectionAdapters
    ...
    class JdbcDriver
      ...
      def connection(url, user, pass)
        db_partition = url.match(/\@[a-zA-Z0-9-]+\./).to_s
        Rails.logger.info("----Creating new database connection : db_partition: #{db_partition} user: #{user}-----")
        Jdbc::DriverManager.getConnection(url, user, pass)
      rescue
        # bypass DriverManager to get around problem with dynamically loaded jdbc drivers
        Rails.logger.info("----Creating new database connection  in rescue : url: #{db_partition} user: #{user}-----")
    ...
    java_import "jdbc_adapter.JdbcConnectionFactory"
