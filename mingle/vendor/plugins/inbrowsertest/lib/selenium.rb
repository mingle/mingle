...
module Selenium
    ...
    class SeleneseInterpreter
        ...
        def initialize(server_host, server_port, browserStartCommand, browserURL)
        ...
        def do_command(verb, args)
          ...
          timeout(@timeout) do
            ...
            if (response.body[0..1] != "OK")
                raise SeleniumCommandError, %Q[
Selenium command #{verb.inspect} with args #{args.inspect} failed here:

#{caller.join("\n")}

========================================
Selenium Request: #{command_string}
Selenium Response: #{result}
========================================
]
            end
          ...
          raise SeleniumCommandError.new("timeout when executing '#{verb}'")
        end
        ...
        def open(url)
          # if absolute prepend the browserURL
          if url[0..0] == '/'
            url = @browserURL + url
          end
          do_command("open", [url, true])
        end
        ...
        def get_value(locator)
            (MingleUpgradeHelper.ruby_1_9? ? get_string("getValue", [locator,]).force_encoding('utf-8') : get_string("getValue", [locator,]))
        end
        ...
        def get_text(locator)
          (MingleUpgradeHelper.ruby_1_9? ? get_string("getText", [locator,]).force_encoding('utf-8') : get_string("getText", [locator,]))
        end
        ...
        #
        # 'timeout' is a timeout in milliseconds, after which this command will return with an error
        def wait_for_page_to_load(timeout=60000)
            do_command("waitForPageToLoad", [timeout,])
        end

        def verify_confirmation(message)
          do_command('verifyConfirmation', [message])
        end
