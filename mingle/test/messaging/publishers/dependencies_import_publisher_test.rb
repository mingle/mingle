...
class DependenciesImportPublisherTest < ActiveSupport::TestCase
  include MessagingTestHelper

  def test_publish_dependencies_import_message
    login_as_admin
    @project1 = create_project(:name => 'project 1', :identifier => 'project_1')
    user = User.first_admin
    publisher = DependenciesImportPublisher.new(user, 12)
    publisher.publish_message
    asynch_request = publisher.asynch_request

    messages = all_messages_from_queue(DependenciesImportProcessor::QUEUE)

    assert_equal 1, messages.size
    assert_equal user.id, messages.first[:user_id]
    assert_equal asynch_request.id, messages.first[:request_id]
    assert_equal 12, messages.first[:import_preview_asynch_request_id]
  end
end
