...
class StringExtTest < ActiveSupport::TestCase
  ...
  def test_html_bold_should_surround_string_with_bold_tags
    assert_equal '<b>abc</b>', 'abc'.html_bold
  end

  def test_bold_should_surround_string_with_mingle_bold_tokens
    assert_equal "#{MingleFormatting::MINGLE_BOLD_MARKER_OPEN}abc#{MingleFormatting::MINGLE_BOLD_MARKER_CLOSE}", 'abc'.bold
  end
  ...
  def test_md5
    assert_equal MD5::md5('ABC').to_s, 'ABC'.md5
  end
