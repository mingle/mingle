...
class UserTest < ActiveSupport::TestCase
  ...
  def test_should_not_be_able_to_create_current_user
    user = User.new(:name => PropertyType::UserType:: CURRENT_USER, :login => 'very_new', :password => MINGLE_TEST_DEFAULT_PASSWORD, :password_confirmation => MINGLE_TEST_DEFAULT_PASSWORD, :email => 'verynew@email.com')
    user.save
    assert_equal "Name cannot be set to #{'(current user)'.bold}", user.errors.full_messages.join
  end

  def test_auth
    assert_equal  User.find_by_login("bob"), User.authenticate("bob", MINGLE_TEST_DEFAULT_PASSWORD)
  ...
  def test_users_created_from_import_which_do_not_have_salt_nor_password_should_fail_authentication
    user = User.new :name => 'New Bob', :email => 'hello@world.com'
    user.login = "newbob"
    user.password = user.password_confirmation = "bobs_secure_password1."
    user.save!
    ActiveRecord::Base.connection.execute SqlHelper.sanitize_sql("UPDATE users SET salt = NULL, password = NULL WHERE id = ?", user.id)
    assert_nil User.authenticate('newbob', 'bobs_secure_password1.')
  end

  def test_password_should_not_restricted_except_can_not_be_empty_when_password_format_is_nil
    Authenticator.password_format = nil
    user = User.new
    user.login = 'bababa'
    user.email = 'bababa@email.com'
    user.name = 'bababaname'
  ...
  def test_password_confirmation_needs_only_be_checked_when_password_is_changed
    Authenticator.password_format = nil
    user = User.new :login => 'bababa', :email => 'bababa@email.com', :name => 'bababaname'
    user.password = user.password_confirmation = 'p'
    user.save!
    user.password = 'ppp'
    assert !user.valid?
  ...
  def test_change_password_should_set_login_access_lost_password_key_to_nil_and_returns_true
    ...
    bob.login_access.generate_lost_password_ticket!
    assert bob.change_password!(:password => 'p@y B1ll', :password_confirmation => 'p@y B1ll')
    assert_nil bob.login_access.lost_password_key
    assert_nil bob.login_access.lost_password_reported_at
  end

  def test_invalid_password_change_should_not_set_login_access_lost_password_key_to_nil_and_returns_false
    bob = User.find_by_login('bob')
    bob.login_access.generate_lost_password_ticket!
    assert_false bob.change_password!(:password => 'pay bill', :password_confirmation => 'pay bill')
    assert_not_nil bob.login_access.lost_password_key
    assert_not_nil bob.login_access.lost_password_reported_at
  end

  def test_validate_email
    u = User.new
    assert_is_valid_email(nil, u)
    assert_is_valid_email("your_name@your_company.com", u)
  ...
  def assert_ok_login_format(login)
    u = User.new
    u.email = "#{'user'.uniquify}@example.com"
    u.name = 'name'
    u.password = u.password_confirmation = "bobs_secure_password1."
    u.login = login
    assert u.save
  ...
  def test_create
    u = User.new
    u.login      = "newbob"
    u.email      = "nonexistingbob@email.com"
    u.name       = 'name'
    u.password = u.password_confirmation = "bobs_secure_password1"
  ...
  def test_password_must_present
    assert_false User.new(:login => 'a', :name => 'A').valid?
  end

  def test_after_create_password_is_hashed
    u = User.new
    u.login      = "newbob"
    u.email      = "newbob@email.com"
    u.name       = 'name'
    u.password = u.password_confirmation = "bobs_secure_password123-"
    assert u.save
    hashed_password = Digest::SHA256.hexdigest(u.salt + Digest::SHA1.hexdigest("mingle--bobs_secure_password123---"))
    assert_equal hashed_password, u.password
  end
  ...
  def test_user_should_be_the_sole_admin_if_user_is_only_one_admin_that_activated
    ...
    assert User.find_by_login("admin").sole_admin?
    assert !User.find_by_login('member').sole_admin?
    assert !User.find_by_login('member').admin?

    User.find_by_login('member').update_attributes(:admin =>  true)
    assert !User.find_by_login("admin").sole_admin?
    assert !User.find_by_login('member').sole_admin?

    User.find_by_login('member').update_attributes(:activated => false)
    assert User.find_by_login("admin").sole_admin?
    assert !User.find_by_login('member').sole_admin?

  end

  def test_cannot_make_last_admin_not_admin
    assert User.find_by_login("admin").sole_admin?
    user = User.find_by_login("admin")
    user.update_attributes(:admin => false)
    assert_equal "Administrator #{user.name} cannot be removed as they are the last admin", user.errors.full_messages.join
  end

  def test_cannot_deactivate_last_admin
    user = User.find_by_login("admin")
    user.update_attributes(:activated => false)
    assert_equal "Administrator #{user.name} cannot be deactivated as they are the last admin", user.errors.full_messages.join
  end
  ...
  def test_password_is_checked_for_basic_quality_when_password_format_is_strict
    assert_equal 'is too short (minimum is 5 characters)', User.create(:password => 'a1.').errors.on(:password)
    assert_equal 'needs at least one digit', User.create(:password => 'banan.').errors.on(:password)
    assert_equal 'needs at least one non-character symbol (e.g. ".", "," or "-")',
    User.create(:password => 'banan1').errors.on(:password)
    assert_equal nil, User.create(:password => 'banan1.').errors.on(:password)
  end
  ...
  def test_should_strip_on_both_password_and_password_confirmation
    user = User.new(:login => 'booha', :password_confirmation => ' 1234566. ', :password => ' 1234566. ')
    assert_equal('1234566.', user.password_confirmation)
    assert_equal('1234566.', user.password)
  end
  ...
  def test_member_of
    ...
    first_project.with_active_project do |proj|
      first_project.add_member(member)
      first_project.remove_member(admin)

      assert member.reload.member_of?(first_project)
      assert !admin.reload.member_of?(first_project)
    end
  end

  def test_member_of_for_auto_enroll_project
    user = create_user!
    with_new_project(auto_enroll_user_type: AutoEnrollUserType::ALL_USERS_ARE_READ_ONLY_MEMBERS) do |project|
      assert user.member_of?(project)

      login_as_admin
      project.disable_auto_enroll
      logout_as_nil

      assert user.member_of?(project.reload)
    end
  end

  def test_membership_role_for_unassociated_project_is_nil
    user = create_user!
    with_new_project do |project|
      assert user.membership_role_for(project).nil?
    end
  end

  def test_membership_role_for_associated_project_is_team_member
    user = create_user!
    with_new_project do |project|
      project.add_member(user)
      assert_equal "Team member", user.membership_role_for(project)
    end
  end

  def test_membership_role_for_autoenroll_read_only_project_is_read_only_team_member
    user = create_user!
    with_new_project(auto_enroll_user_type: AutoEnrollUserType::ALL_USERS_ARE_READ_ONLY_MEMBERS) do |project|
      assert_equal "Read only team member", user.membership_role_for(project)
    end
  end
...
EXPECTED
  ...
  def test_accessible_projects_should_not_include_hidden_projects
    ...
    accessible_project_names = User.find_by_login('admin').accessible_projects.collect(&:name)
  ...
  def test_mingle_admin_should_always_be_project_admin
    mingle_admin = User.find_by_login('admin')
  ...
  def test_system_user_does_not_go_to_user_list
    ...
    assert_not_include user, User.all
  end
  ...
  def test_deletable_users_should_not_contain_system_users
    user = User.create_or_update_system_user(:login => 'sys', :name => 'System admin')
    assert_not_include user, User.deletable_users
  end
  ...
  def test_project_role_candidates_should_return_readonly_role
    user = create_user!(light: true)
    assert_equal [MembershipRole[:readonly_member]], user.project_role_candidates
  end

  def test_project_role_candidates_should_return_all_project_roles
    user = create_user!
    expected_roles = [ MembershipRole[:full_member], MembershipRole[:readonly_member], MembershipRole[:project_admin]]
    assert_equal expected_roles, user.project_role_candidates
  end
end
