...
class MultitenancyTest < ActiveSupport::TestCase
  ...
  def test_activate_tenant_with_correct_connection_pool
    Multitenancy.add_tenant('tenantx', "database_username" => 'second_schema', 'db_config' => {'url' => 'new database url'})
    db_url = Multitenancy.activate_tenant('tenantx') do
      ActiveRecord::Base.connection_pool.spec.config[:url]
    end
    assert_equal 'new database url', db_url
  ensure
  ...
  def test_activate_tenants_should_activate_each_tenant_and_execute_the_block_on_each_tenant
    ...
    assert_sort_equal ['first', 'second'], names
  end

  def test_setup_block_get_called_on_add_tenant_if_called
    call_from_namespace = nil
    Multitenancy.add_tenant('test', "database_username" => 'schema' ) do
  ...
  def test_tenant_name_uniqueness_should_be_case_insensitive
    Multitenancy.add_tenant('test', "database_username" => 'schema' )
    assert Multitenancy.tenant_exists?('Test')
  end
  ...
  def test_tenant_active_will_switch_app_namespace
    old_app_namespace = MingleConfiguration.app_namespace
  ...
  def test_tenant_active_should_put_prev_tenant_name_back_mdc_when_deactivate_the_tenant
    does_not_work_without_jruby do
      ...
      assert_equal nil,  org.apache.log4j.MDC.get("tenant")
    end
  end
  ...
  def test_tenant_should_not_exist_if_add_tenant_with_block_failed
    assert_raise RuntimeError do
      Multitenancy.add_tenant('site1', "database_username" => 'site1_schema') do
        raise "hello world"
      end
    end
    assert !Multitenancy.tenant_exists?('site1')
  ...
  def test_overwrite_default_mingle_properties
    Multitenancy.add_tenant('hello', "database_username" => 'site1_schema', "mingle.config.asset_host" => "http://getmingle.io", "mingle.config.debug" => "true")
  ...
  def test_should_ignore_wrong_configuration
    Multitenancy.add_tenant('hello', "database_username" => 'site1_schema', "mingle.config.not_exist" => "true", "mingle.config.debug" => "true")
    activate_tenant("hello") do
      assert MingleConfiguration.debug?
  ...
  def test_cannot_overwrite_app_namespace
    Multitenancy.add_tenant('hello', "database_username" => 'site1_schema', "mingle.config.app_namespace" => "something wrong")
    activate_tenant("hello") do
