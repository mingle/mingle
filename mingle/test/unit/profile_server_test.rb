...
class ProfileServerTest < ActiveSupport::TestCase
  ...
  def test_user_attributes_changes_relayed_to_profile_server
    ...
    assert_equal user.login_access.last_login_at.iso8601,  post_attrs['last_login_at']
  end
  ...
  def test_should_retry_user_creation_upto_4_times_on_network_error
    ...
    assert_raises(ProfileServer::NetworkError) { create_user! }
  ...
  def test_update_saml__metadata
    time = Time.now
    ProfileServer.update_last_data_exported_on(time)
    assert_equal 1, @http_stub.requests.size
    assert_equal :post, @http_stub.last_request.http_method
    assert_equal "https://profile_server/organizations/parsley/last_data_exported_on.json", @http_stub.last_request.url
    post_attrs = JSON.parse(@http_stub.last_request.body)["timestamp"]
    assert_equal time.strftime('%d-%b-%y %H:%M:%S'), Time.parse(post_attrs).strftime('%d-%b-%y %H:%M:%S')
  end
end
