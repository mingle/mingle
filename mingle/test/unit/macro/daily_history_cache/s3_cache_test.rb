...
class S3CacheTest < ActiveSupport::TestCase
  def self.storage_configured?
    ENV['AWS_ACCESS_KEY_ID'] && ENV['S3_BUCKET_NAME']
  end

  if !storage_configured?
    def run(*args)
      putc 'S'
      at_exit { puts "WARNING: #{name} does not work without AWS_ACCESS_KEY_ID & S3_BUCKET_NAME environment variables, skipped..." }
    end
  end

  def setup
    login_as_member
    MingleConfiguration.daily_history_cache_bucket_name = ENV['S3_BUCKET_NAME']
    @cache = DailyHistoryCache.s3_store("s3_cache_test")
  end

  def teardown
    MingleConfiguration.daily_history_cache_bucket_name = nil
    @cache.clear_store
  end
  ...
  def test_save_should_not_call_block_if_data_for_specific_date_is_in_s3
    @cache.save("2009-05-14".to_date) do
      [0,1]
    end
    Cache.flush_all
