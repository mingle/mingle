...
class DatabaseTest < ActiveSupport::TestCase
  ...
  def test_migration_when_a_new_migration_added_but_it_is_not_last_migration
    for_postgresql do
      with_new_migration(Database.send(:last_migration_version) - 1) do
        Database.send(:reset_last_migration_version)
  ...
  def test_is_using_mysql_should_tell_if_using_mysql_database
    original_configurations = ActiveRecord::Base.configurations

    ActiveRecord::Base.configurations = {'production' => {'driver' => 'mysql'}}
    assert Database.is_mysql?
    ActiveRecord::Base.configurations = {'production' => {'driver' => 'postgres'}}
    assert !Database.is_mysql?
  ...
  def test_is_using_mysql_should_raise_exception_if_unable_to_fix_determine_AR_configuration
    original_configurations = ActiveRecord::Base.configurations.dup

    ActiveRecord::Base.configurations.instance_eval do
      def [](key)
        raise License::DatabaseVendorDetectionError.new('Boom')
      end
    end
    assert_raise(License::DatabaseVendorDetectionError) { Database.is_mysql? }
  ensure
    ActiveRecord::Base.configurations = original_configurations
  end
