...
config.active_record.colorize_logging = false if !ENV['CC_HOSTNAME'].nil?
...
unless Messaging::Processor.method_defined?(:run_once_with_pre_deliver_messaging)
  Messaging::Processor.send(:include, ::Messaging::DeliverMessages)
end

# ensure that we run with all features unless explicitly deactivated in tests
ENV.delete("deactivate.features")
